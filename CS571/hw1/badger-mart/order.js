...
function calculateSalesTax() {
	...
	return getSalesTaxRateForState(state) * calculateSubtotal();
}

function getSalesTaxRateForState(state) {
	for (let targetState in SALES_TAX) {
		if (state === targetState) {
			return parseFloat(SALES_TAX[targetState]);
		}
	}
}
...
document.getElementById("btn-checkout").addEventListener("click", () => {
	...
	const salesTax = parseFloat(calculateSalesTax().toFixed(2));
